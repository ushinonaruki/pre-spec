...
export const UI_TEXT = {
  ...
  startScreen: {
    ...
    markersLabel: 'marker 定義ファイル（任意）',
    markersFileButton: 'pre-spec.markers.json を選ぶ',
    markersFileSelected: (name: string) => `選択中: ${name}`,
    markersFileHint: '選択しない場合、skip marker のみ有効です。',
    markersFileNameError: 'marker 定義ファイル名が標準形式ではありません。pre-spec.markers.json を選択してください。',
    markersFileError: 'marker 定義ファイルの読み込みに失敗しました。JSON の内容を確認してください。',
    startButton: '開始',
