...
`src/config/pre-spec.skip-reasons.json` でスキップ理由の文言・AI への指示を変更できる。

**その他のマーカー**（`src/config/pre-spec.markers.json` で定義）:
...
API は `http://localhost:3001` で起動する。UI は [`frontend-web/`](../frontend-web/) を参照。
